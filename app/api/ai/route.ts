...
export async function POST(request: Request) {
    try {
        ...
        // 最初の会話かどうかを判定（conversationHistoryが空またはundefined）
        ...
        if (isFirstMessage) {
            ...
            salon_context = relatedSalons && relatedSalons.length > 0 ? relatedSalons.map((salon) => {
            ...
            company_context = relatedCompany && relatedCompany.length > 0 ? relatedCompany.map((company) => {
            ...
            以下の情報でユーザーの質問に「回答形式」に従って必ずJSON形式で回答してください。
            指定された箇所以外は通常テキストで出力し、Markdown形式やコードブロックは使用しないでください。
            IDなどは処理のみに使用するのでrecom_id以外の箇所では記述しないでください。
            ...
            回答形式：
            {
                ...
                "quest" : string // アドバイスや追加情報、よりよい回答をするための質問などがあれば記載、なければ空文字列、Markdown形式でわかりやすいよう簡潔に記述
            }
            `;
        } else {
            systemPrompt = `
            あなたは就職活動中の求職者向けのAIコンサルタントです。
            最初のメッセージで提供されたデータベース情報を参照して、ユーザーの質問に答えてください。
        ...
        if (conversationHistory && conversationHistory.length > 0) {
            conversationHistory.forEach((msg: { role: string, content: string }) => {
                conversationContent.push({
                    ...
                    parts: [{ text: msg.role === 'user' ? `ユーザーの質問：\n${msg.content}` : msg.content }]
                });
            });
        }
